...
contract GovernorZK is IGovernor {
    ...
    ) external override onlyBoardMember returns (bytes32) {
        ...
        validProposal(proposalId) 
    {
        ...
        for (uint256 i = 0; i < actions.targets.length; i++) {
            (bool success, ) = actions.targets[i].call{value: actions.values[i]}(
                actions.calldatas[i]
            );
    ...
    override 
    validProposal(proposalId) 
    returns (IGovernor.ProposalState)  // ← Explicitly use IGovernor's
{
    ProposalTypes.ProposalCore storage proposal = proposals[proposalId];
        ...
        validProposal(proposalId) 
    {
        ProposalTypes.ProposalCore storage proposal = proposals[proposalId];
        require(
            msg.sender == proposal.proposer || boardManagement.isChairman(msg.sender),
    ...
    function getProposalDetails(bytes32 proposalId) 
        ...
        view 
        returns (
            ProposalTypes.ProposalCore memory proposal,
            ProposalTypes.VotingResults memory results
        ) 
    {
        return (proposals[proposalId], votingResults[proposalId]);
    }

    function getProposalActions(bytes32 proposalId) 
        external 
        view 
        returns (ProposalTypes.ProposalActions memory) 
    {
        return proposalActions[proposalId];
    }

    function getEncryptedBudget(bytes32 proposalId) 
        external 
        view 
        returns (ProposalTypes.EncryptedBudget memory) 
    {
    ...
    function decryptBudget(
        ...
        // bytes memory proof
    ) external onlyBoardMember validProposal(proposalId) {
