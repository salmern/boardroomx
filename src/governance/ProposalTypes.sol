...
library ProposalTypes {
    ...
    event VoteCast( // For anonymous voting
        bytes32 indexed proposalId,
    ...
    event BudgetEncrypted(bytes32 indexed proposalId, bytes32 budgetHash, uint256 timestamp);

    event BudgetDecrypted(bytes32 indexed proposalId, string budgetDetails, uint256 timestamp);

    // Helper functions
    function getQuorumRequirement(ProposalType proposalType) internal pure returns (uint256) {
    ...
    function getVotingPeriod(ProposalType proposalType) internal pure returns (uint256) {
    ...
    function getExecutionDelay(ProposalType proposalType) internal pure returns (uint256) {
