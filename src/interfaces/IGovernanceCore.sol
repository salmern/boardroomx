...
interface IVerifier {
    function verifyProof(uint256[2] memory a, uint256[2][2] memory b, uint256[2] memory c, uint256[] memory input)
        external
        view
        returns (bool);
}
...
interface IGovernor {
    ...
    event ProposalCreated(
        bytes32 indexed proposalId, address indexed proposer, uint256 startTime, uint256 endTime, string description
    );

    event VoteCast(address indexed voter, bytes32 indexed proposalId, uint8 support, uint256 weight, string reason);
...
interface IEncryptedProposal {
    ...
    function submitEncryptedBudget(bytes32 proposalId, bytes32 budgetHash, bytes memory encryptedData) external;

    function decryptBudget(bytes32 proposalId, string memory decryptedBudget, bytes memory proof) external;
}
